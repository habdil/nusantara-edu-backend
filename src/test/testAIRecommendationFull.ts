...
import { ImplementationStatus } from '../types/aiTypes';
...
async function testAIRecommendationFull() {
  ...
  try {
    ...
    if (getResult.success && getResult.data.length > 0) {
      ...
      const singleResult = await aiRecommendationService.getRecommendationById(
        firstRec.id,
        testSchool.id
      );
      ...
      const updateResult = await aiRecommendationService.updateRecommendation(
        firstRec.id,
        testSchool.id,
        {
      ...
      if (getResult.data.length > 1) {
        ...
        const bulkResult = await aiRecommendationService.bulkUpdateStatus(
          idsToUpdate,
          testSchool.id,
    ...
    const dbRecords = await prisma.aiRecommendation.findMany({
...
export default testAIRecommendationFull;