...
export class AssetController {
  ...
  getAssets = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { error, value } = getAssetsValidation.validate(req.query);
      if (error) {
        res.status(400).json({
          success: false,
  ...
  getAssetsByCategory = async (req: Request, res: Response): Promise<void> => {
    try {
      if (!req.user?.schoolId) {
        res.status(400).json({
          success: false,
  ...
  addAsset = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { error, value } = createAssetValidation.validate(req.body);
      if (error) {
        res.status(400).json({
          success: false,
  ...
  updateAsset = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Validate request body
      const { error, value } = updateAssetValidation.validate(req.body);
      if (error) {
        res.status(400).json({
          success: false,
          message: 'Data tidak valid',
  ...
  getAllMaintenanceRecords = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { error, value } = getMaintenanceValidation.validate(req.query);
      if (error) {
        res.status(400).json({
          success: false,
          message: 'Parameter tidak valid',
  ...
  addMaintenanceRecord = async (req: Request, res: Response): Promise<void> => {
    try {
      if (!req.user?.schoolId) {
        res.status(400).json({
          success: false,
      ...
      // Validate request body
      const { error, value } = createMaintenanceValidation.validate(req.body);
      if (error) {
        res.status(400).json({
          success: false,
          message: 'Data tidak valid',
